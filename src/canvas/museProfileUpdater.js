...
// (the summary prompt) the muse's profile gets updated lives here and only
 ...
 * muse_increment_interaction), then only refreshes the profile once the
 * song's total count has actually reached N. >= rather than a strict ===
 * guards against ever getting stuck if a previous refresh failed after
 * incrementing but before the reset-to-0 landed.
 * @returns {Promise<object|null>} the updated, fused profile object on
 *   success, null if it didn't run or failed (never throws — a failed
 *   background refresh shouldn't break the turn that triggered it).
 */
export async function recordMuseTurnAndMaybeUpdateProfile({ songId, existingProfile }) {
  const { data: newCount, error: incrementError } = await incrementMuseInteraction(songId);
  if (incrementError || newCount < MUSE_PROFILE_UPDATE_EVERY) return null;

  const { data: recent, error: fetchError } = await getRecentMuseUserMessages(songId, MUSE_PROFILE_UPDATE_EVERY);
  ...
  try {
    ...
    const updatedProfile = await summarizeMuseProfile({ existingProfile, userMessages });
    const { error } = await saveMuseSummary(songId, updatedProfile);
