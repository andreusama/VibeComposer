    ...
    <div class="body">
      ...
      ${renderTagSection('texture', state.texture)}
        <label class="easy-toggle">
          <input type="checkbox" id="easy-mode" ${state.easyMode ? 'checked' : ''} />
          <div class="checkbox-box ${state.easyMode ? 'checked' : ''}" id="checkbox-visual">
            ${state.easyMode ? '✓' : ''}
          </div>
          <div>
            <div class="easy-label-main">easy chords only</div>
            <div class="easy-label-sub">open positions · no barre chords · beginner friendly</div>
          </div>
        </label>
...
export function attach(state) {
    ...
    if (easyCheck) {
        easyCheck.addEventListener('change', (e) => {
            setState({easyMode: e.target.checked});
        });
    }
}
